...
<html>
<head>
<title>Bhuiyan Group | Factory | Product Made</title>
                                ...
                                <div class="panel panel-success" style="margin-top: 10px">
                                        <div class="row">
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                <div class="div">
                                                	<h3 class="alert bg-primary">Product Made</h3>
                                                </div>
                                ...
                                <tbody>
                                	...
                                	$today = date('d-m-Y');
                                		...
                                		while($dist= $dis->fetch_assoc()):
                                			...
                                			$madePrName = $_POST['madePrName'];
                                			$madePrQuantity = $_POST['madePrQuantity'];
                                			$madePrUnit = $_POST['madePrUnit'];
                                			$madePrStatus = "Pending";
                                			$madePrDate = date('d-m-Y');

                                			if(empty($madePrName) || (empty($madePrQuantity) && $madePrQuantity!=0) || empty($madePrUnit)){
											$message = "<p class='alert alert-danger'>Field Must Not Be Empty</p>";

											}else{
												$pr = $conn->query("SELECT * FROM product WHERE prName = '$madePrName' ");

												$d = $conn->query("SELECT categoryName FROM product WHERE prName='$madePrName'");
												$e=$d->fetch_assoc();
												$p = $e['categoryName'];
												...
												if($prNameCheak==$madePrName AND $prUnitCheak== $madePrUnit){
													 $perUnitPrice = $prod['prFixedPrice']/$prod['prQuantity'];
													 $totalPrice = $perUnitPrice*$madePrQuantity;

													$conn->query("INSERT INTO madeproduct (madePrName,madePrQuantity,madePrUnit,perUnitPrice,totalPrice,madePrStatus,madePrDate,proCat,color) VALUES('$madePrName','$madePrQuantity','$madePrUnit','$perUnitPrice','$totalPrice','$madePrStatus','$madePrDate','$p','red')");
											
												$message = "<p class='alert alert-success'>Successfully added Made Product!</p>";
												}else{
											...
											echo $message;
                                						...
                                						$product = $dist['prName'];
				                                		$colo = $conn->query("SELECT * FROM madeproduct WHERE madePrDate='$today' AND madePrName='$product'");
	                                ...
	                                <tr>
	                                    ...
	                                    <td class="text-capitalize">
	                                    	<input name="madePrName"  value="<?php echo $dist['prName']?>" type="text" class="form-control text-capitalize" readonly>
	                                    </td>
	                                    <td>
                                            <input  name="madePrQuantity"  type="number" min="0" value="">
                                        </td>
	                                    <td class="text-capitalize">
	                                    	
											
											<input name="madePrUnit"  value="<?php echo $dist['prUnit']?>" type="text" class="form-control" readonly>
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="product-status-wrap">
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                <?php
                                	$made = $conn->query("SELECT * FROM madeproduct WHERE madePrStatus='Pending' AND proCat='$categor' ORDER BY sl DESC");
                                	$i=1;
                                	while($madePr=$made->fetch_assoc()):
                                ?>
                                <tr>
                                    <td><?php echo $i++;?></td>
                                    <td class="text-capitalize"><?php echo $madePr['madePrName']?></td>
                                    <td><?php echo $madePr['madePrQuantity']?></td>
                                    <td class="text-capitalize"><?php echo $madePr['madePrUnit']?></td>
                                    <td><?php echo $madePr['perUnitPrice']?> <span>TK</span></td>
                                    <td><?php echo $madePr['totalPrice']?> <span>TK</span></td>
                                    <td>
                                       <a onclick="javascript: return confirm('Please Confirm All Ok');" href="inc/madePrConfirm.php?sl=<?php echo $madePr['sl']?>&key=<?php echo $sl?>"><button data-toggle="tooltip" title="All Ok" class="pd-setting-ed"><i class="fa fa-check" aria-hidden="true"></i></button></a>
			                            <a onclick="javascript: return confirm('Please Confirm Delation');" href="inc/madePrDelete.php?sl=<?php echo $madePr['sl']?>&key=<?php echo $sl?>"><button data-toggle="tooltip" title="Delete" class="pd-setting-ed"><i class="fa fa-trash-o" aria-hidden="true"></i></button></a>
                                    </td>
                                </tr>
                                <?php endwhile;?>
                                </tbody>
                        ...
                        <div class="sparkline13-list">
                            <div class="sparkline13-hd">
                                <div class="main-sparkline13-hd">
                                    <h3 class="alert bg-success"><b>Product Made List</b></h3>
                                </div>
                            </div>
                                    ...
                                    <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                        ...
                                        <tbody>
											...
											if(empty($_SESSION['facTaxUser']) AND empty($_SESSION['facTaxPass']) ){
												$produ = $conn->query("SELECT * FROM madeproduct WHERE (madePrStatus = 'Confirmed' AND madePrDate='$today' AND proCat='$categor') ORDER BY sl DESC");
											}else{
												$rr = $conn->query("SELECT * FROM madeproduct WHERE (madePrStatus = 'Confirmed' AND madePrDate='$today' AND proCat='$categor')");
												$rrr = $rr-> num_rows;
												 $rrrr = ceil(($rrr*30)/100);

												$produ = $conn->query("SELECT * FROM madeproduct WHERE (madePrStatus = 'Confirmed' AND madePrDate='$today' AND proCat='$categor') ORDER BY sl DESC LIMIT $rrrr");
											}
			                                ...
			                                <tr>
			                                    ...
			                                    <td><?php echo $expen['perUnitPrice']?> <span>TK</span></td>
