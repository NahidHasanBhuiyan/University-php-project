...
<html>
<head>
<title>Bhuiyan Group | Factory | Product Distribution</title>
                                ...
                                <tbody>
                                	...
                                	$today = date('d-m-Y');
                                		...
                                		while($dist= $dis->fetch_assoc()):
                                			...
                                			if(empty($prDistPrName) || (empty($prDistQuantity) && $prDistQuantity!=0) || empty($prDistUnit)){
											$message = "<p class='alert alert-danger'>Field Must Not Be Empty</p>";
												...
												if($prNameCheak==$prDistPrName AND $prUnitCheak== $prDistUnit){
													...
													$conn->query("INSERT INTO productdistribution (prDistOutleName,prDistPrName,prDistQuantity,prDistUnit,distPerUnitPrice,distTotalPrice,prDistStatus,prDistDate,catName,color) VALUES('$prDistOutleName','$prDistPrName','$prDistQuantity','$prDistUnit','$distPerUnitPrice','$distTotalPrice','$prDistStatus','$prDistDate','$p','red')");
                                		...
                                		$colo= $conn->query("SELECT * FROM productdistribution WHERE prDistDate='$today' AND dist='0' AND prDistOutleName='$outlet' AND prDistPrName='$product'");
	                                ...
	                                <tr>
	                                    ...
	                                    <td>
                                            ...
                                            <input  name="demo2"   type="number" min="0" value="">
                                        </td>
	                                    <td class="text-capitalize">
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="product-status-wrap">
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                	<?php
                                		$dis = $conn->query("SELECT * FROM productdistribution WHERE prDistStatus='Pending' AND dist='0' AND prDistOutleName='$outlet' AND catName='$category' ORDER BY sl DESC");
                                		$i=1;
                                		while($dist= $dis->fetch_assoc()):
                                	?>
	                                <tr>
	                                    ...
	                                    <td>
	                                        <a onclick="javascript: return confirm('Please Confirm All Ok');" href="inc/prDistConfirm.php?sll=<?php echo $dist['sl']?>&sl=<?php echo $sl?>&key=<?php echo $key?>"><button data-toggle="tooltip" title="All Ok" class="pd-setting-ed"><i class="fa fa-check" aria-hidden="true"></i></button></a>
			                           		 <a onclick="javascript: return confirm('Please Confirm Delation');" href="inc/prDistDelete.php?sll=<?php echo $dist['sl']?>&sl=<?php echo $sl?>&key=<?php echo $key?>"><button data-toggle="tooltip" title="Delete" class="pd-setting-ed"><i class="fa fa-trash-o" aria-hidden="true"></i></button></a>
	                                    </td>
	                                </tr>
	                            <?php endwhile;?>
                                    ...
                                    <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                        ...
                                        <tbody>
											...
											if(empty($_SESSION['facTaxUser']) AND empty($_SESSION['facTaxPass']) ){
												$ddd = $conn->query("SELECT * FROM productdistribution WHERE (prDistStatus = 'Confirmed' AND dist='0' AND prDistDate='$today' AND prDistOutleName='$outlet' AND catName='$category') ORDER BY sl DESC");
											}else{
												$rr = $conn->query("SELECT * FROM productdistribution WHERE (prDistStatus = 'Confirmed' AND dist='0' AND prDistDate='$today' AND prDistOutleName='$outlet' AND catName='$category') ");
												$rrr = $rr-> num_rows;
												 $rrrr = ceil(($rrr*30)/100);

												$ddd = $conn->query("SELECT * FROM productdistribution WHERE (prDistStatus = 'Confirmed' AND dist='0' AND prDistDate='$today' AND prDistOutleName='$outlet' AND catName='$category') ORDER BY sl DESC LIMIT $rrrr");
											}
