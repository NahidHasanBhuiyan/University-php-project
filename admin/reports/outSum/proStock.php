...
<html>
<head>
<title>Bhuiyan Group | Reports | Outlet Summary | Product Stock</title>
...
<!--market updates updates-->
	 <div class="container-fluid" >
                            ...
                            <div class="sparkline13-graph">
                                <div class="datatable-dashv1-list custom-datatable-overright">
                                    ...
                                    <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                        ...
                                        <tbody>
        <?php
		$ccc = $conn->query("SELECT  outName FROM croutlet");
		$nu = 1;
		$summ=0;
		while($d=$ccc->fetch_assoc()):
			$product = $d['outName'];

			$c = $conn->query("SELECT DISTINCT prDistPrName,prDistUnit FROM productdistribution WHERE prDistOutleName='$product'AND outletStatus='Confirmed' ORDER BY sl DESC");
		...
		$mmm = $conn->query("SELECT MAX(stockDate) as stockDate from stock WHERE stockPrName='$stockPrName' AND stockOutletName='$product' AND stockStatus='Confirmed' AND stockDate<'$today'");
		$mmmm = $mmm->fetch_assoc();
		 $maax = $mmmm['stockDate'];

		$uu  = $conn->query("SELECT * FROM stock WHERE stockDate='$maax' AND stockStatus='Confirmed' AND stockOutletName='$product' AND stockPrName='$stockPrName'");
		$uuu = $uu->fetch_assoc();
		...
		$sss = $conn->query("SELECT * FROM stock WHERE stockDate='$today' AND stockStatus='Confirmed' AND stockOutletName='$product' AND stockPrName='$stockPrName'");
		$ssss = $sss->fetch_assoc();
		$stockQuantityUpdate = $ssss['stockQuantity'];



		$rec = $conn->query("SELECT SUM(prDistQuantity) AS prq FROM productdistribution WHERE prDistOutleName='$product' AND prDistPrName='$stockPrName' AND outletStatus='Confirmed' AND prDistDate='$today' ");
													$recpr = $rec->fetch_assoc();
													$receive = $recpr['prq'];
													$was = $conn->query("SELECT SUM(wasOutQuantity) AS wasq FROM wastageoutlet WHERE wasOutlet='$product' AND wasOutPrName='$stockPrName' AND wasStatus='Confirmed' AND wasDate='$today' ");
													$wasquery = $was->fetch_assoc();
													$wastage = $wasquery['wasq'];

													$dist = $conn->query("SELECT SUM(prDistQuantity) as distq FROM productdistribution WHERE prFrom='$product' AND prDistPrName='$stockPrName' AND outletStatus='Confirmed' AND prDistDate='$today'");
		...
		$summ = $summ+$sum;
                                            ...
                                            <tr>
                                                <td></td>
                                                <td><?php echo $nu++;?></td>
                                                <td class="text-capitalize"><?php echo $product?></td>
                                                <td><?php echo $sum?> <span>TK</span></td>
                                        ...
                                        <div class="row">
		                                	<div class="col-md-6">
		                                    <h3 class="alert alert-info"><b class="text-left">Total Product Stock :</b ></h3>
		                                    </div>
		                                    <div class="col-md-6">
		                                    <h3 class="alert alert-info"><span class="text-right"><b>BDT <?php echo $summ;?></b></span></h3>
		                                    </div>
                                   		 </div>
                                    </table>
