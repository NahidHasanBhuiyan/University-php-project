...
<html>
<head>
<title>Bhuiyan Group | Admin | Best Preformer</title>
            ...
            <!--header start here-->
				<div class="header-main">
					<div class="header-left" style="background-color: #00bfff; ">
							<div class="logo-nam">
									 <h1 class="text-center" style="padding: 5px; color: #fff"><b>Best Performer</b></h1> 
...
<!--market updates updates-->
	 <div class="container-fluid" >
			...
			<div class="row">
				<div class="col-md-4">
					<div class="panel panel-success"> 
			    		  ...
			    		  <div class="panel-body bg-success">
			    		  	<p class="panel-heading text-right"><strong>Best Performing Outlet</strong></p>
			    		  	<br><br>
			    		  	<?php
			    		  		$a = $conn->query("SELECT MAX(todaySold) as maxSold FROM soldproduct WHERE soldDate='$today' AND soldStatus='Confirmed' GROUP BY sl");
			    		  		while($b=$a->fetch_assoc()){
			    		  			 $e=$b['maxSold'];

			    		  		$c=$conn->query("SELECT soldOutlet,todaySold FROM soldproduct WHERE todaySold='$e'");
			    		  		$d=$c->fetch_assoc();
			    		  		...
			    		  		if(isset($d['soldOutlet'])){
			    		  			
			    		  		

			    		  	?>
			    		      <h4 class="text-capitalize" style="color: #00bfff"><strong><?php echo $d['soldOutlet']?></strong></h4>
			    		      <?php };?>
			    		  </div> 
			    		  <div class="panel-footer" style="background-color: #00bfff; color: #fff">
			    		      <h4 class="text-right " >BDT <span class="counter"><?php echo $e?></span></h4>
			    		  </div>
			    	</div> 
    			</div>
    			<div class="col-md-4">
					<div class="panel panel-success"> 
			    		  ...
			    		  <div class="panel-body bg-success">
			    		  	<p class="panel-heading text-right"><strong>Best Selling Product</strong></p>
			    		  	<br><br>
			    		  	<?php
			    		  		$f = $conn->query("SELECT MAX(totalQuantity) as maxQuantity FROM soldproduct WHERE soldDate='$today' AND soldStatus='Confirmed' GROUP BY sl");
			    		  		while($g=$f->fetch_assoc()){
			    		  				 $h=$g['maxQuantity'];

			    		  		$i=$conn->query("SELECT soldPrName,soldPrUnit FROM soldproduct WHERE totalQuantity='$h'");
			    		  		$j=$i->fetch_assoc();
			    		  		}
			    		  		
			    		  		
			    		  		if(isset($j['soldPrName']) ){
			    		  	...
			    		  	?>
			    		      <h4 class="" style="color: #00bfff"><strong><?php echo $j['soldPrName']; }?></strong></h4>
			    		  </div> 
			    		  <div class="panel-footer" style="background-color: #00bfff; color: #fff">
			    		      <h4 class="text-right " ><span class="counter"><?php echo $h?></span> <?php
			    		      if(isset($j['soldPrUnit'])){
			    		      	echo $j['soldPrUnit']; 
			    		      }
			    		       ?></h4>
			    		  </div>
    			...
    			<div class="col-md-4">
					<div class="panel panel-success"> 
			    		  ...
			    		  <div class="panel-body bg-success">
			    		  	<p class="panel-heading text-right"><strong>Lowest Wastage</strong></p>
			    		  	<br><br>
			    		  	<?php
			    		  		$k = $conn->query("SELECT MIN(totalWastage) as minWas FROM wastageoutlet WHERE wasDate='$today' AND wasStatus='Confirmed' GROUP BY sl");
			    		  		while($l=$k->fetch_assoc()){
			    		  			$m=$l['minWas'];

			    		  			$n=$conn->query("SELECT wasOutlet,totalWastage FROM wastageoutlet WHERE totalWastage='$m'");
			    		  			$o=$n->fetch_assoc();
			    		  			


			    		  		}
			    		  		if(isset($o['wasOutlet'])){
			    		  	...
			    		  	?>
			    		      <h4 class="text-capitalize" style="color: #00bfff"><strong><?php echo $o['wasOutlet']; }?></strong></h4>
			    		  </div> 
			    		  <div class="panel-footer" style="background-color: #00bfff; color: #fff">
			    		      <h4 class="text-right " >BDT <span class="counter"><?php echo $o['totalWastage']?></span></h4>
			    		  </div>
			    	</div> 
    			</div>
		...
		<p class="alert bg-primary " style="text-align: center; margin-top:100px">Powered By @ NAHID HASAN BHUIYAN & TEAM</p>
