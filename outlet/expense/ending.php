...
<html>
<head>
<title>Diana | <?php echo $_SESSION['outName'];?> | Ending balance & Reserve</title>
            ...
            <!--header start here-->
				<div class="header-main">
					<div class="header-left" style="background-color: #00bfff; ">
							<div class="logo-nam">
									 <h1 class="text-center" style="padding: 5px; color: #fff"><b>Ending Balance</b></h1> 
                                ...
                                <div class="panel panel-success" style="margin-top: 10px">
                                	<?php
                                		...
                                		if(isset($_POST['addCash'])){
                                			...
                                			$endingReserve = $_POST['endingReserve'];
                                			$dat = date('d-m-Y');
                                			
                                			$cashStatus = "Pending";

                                			if(empty($endingBalance) ){
											$message = "<p class='alert alert-danger'>Field Must Not Be Empty</p>";

											}else{
												$totalCash = $endingBalance+$endingReserve;

												$soo = $conn->query("SELECT SUM(soldTotal) AS sold FROM soldproduct WHERE soldOutlet='$outlet' AND soldDate='$today'");
												$sooo=$soo->fetch_assoc();
												echo $sold = $sooo['sold'];

												$expp = $conn->query("SELECT SUM(OutExpAmount) AS expense FROM outletexpense WHERE expOutlet='$outlet' AND outExpDate='$today'");
												$exppe= $expp->fetch_assoc();
												echo $expense = $exppe['expense'];

												$advpai1 = $conn->query("SELECT SUM(orderAmount) AS amount FROM advanceorder WHERE outlet='$outlet' AND orderDate=delivered AND orderDate='$today'");
												$advpaid1 = $advpai1->fetch_assoc();
												$amount = $advpaid1['amount'];

												$advpai2 = $conn->query("SELECT SUM(orderPaidBill) AS paid FROM advanceorder WHERE outlet='$outlet' AND orderDate!=delivered AND orderDate='$today'");
												$advpaid2 = $advpai2->fetch_assoc();
												$paid = $advpaid2['paid'];


												$advpai3 = $conn->query("SELECT SUM(orderDueBill) AS due FROM advanceorder WHERE outlet='$outlet' AND orderDate!=delivered AND delivered='$today'");
												$advpaid3 = $advpai3->fetch_assoc();
												$due = $advpaid3['due'];
												
												
												

												echo $expected = ($sold+$paid+$amount+$due)-$expense;

												if($expected>$totalCash){
													$gap = $expected-$totalCash;
												}else{
													$gap = $totalCash-$expected;
												}

												

												$conn-> query("INSERT INTO cash (cashOutlet,endingBalance,endingReserve,totalCash,cashDate,cashStatus,cashMonth,expectedCash,cashGap) VALUES ('$outlet','$endingBalance','$endingReserve','$totalCash','$dat','$cashStatus','$month','$expected','$gap')");
												$message = "<p class='alert alert-success'>Successfully added Cash Counter!</p>";
											}
											echo $message;
								...
								<form action="" method="POST">
                                        ...
                                        <div class="row">
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                <div class="div">
                                                	<h3 class="alert alert-success">Cash Counter</h3>
                                                </div>
                                                    
                                                    
                                                    <div class="form-group" style="margin-top: 8px;">
                                                    	<label for=""><i class="fa fa-money" aria-hidden="true"></i> Ending Balance (BDT) <span style="color: red">*</span></label>
                                                        
                                                        <input name="endingBalance" type="text" class="form-control" placeholder="Ex: 20000">
                                                    </div>
                                                    <div class="form-group" style="margin-top: 8px;">
                                                    	<label for=""><i class="fa fa-money" aria-hidden="true"></i> Cash Reserve (BDT)</label>
                                                        
                                                        <input name="endingReserve" type="text" class="form-control" placeholder="Ex: 500">
                                                    </div>
                                        ...
                                        <div class="row" style="margin-top: 8px">
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                <div class="text-center mg-b-pro-edt custom-pro-edt-ds">
                                                    <input name="addCash" type="submit" value="Add Cash Counter" class="btn btn-success">
                        ...
                        <div class="sparkline13-list">
                            <div class="sparkline13-hd">
                                <div class="main-sparkline13-hd">
                                    <h3 class="alert alert-success"><b>Cash Counter List</b></h3>
                                </div>
                            </div>
                                    ...
                                    <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                        ...
                                        <tbody>
                                        	<?php
                                        		...
                                        		$Tcash = $conn->query("SELECT * FROM cash WHERE cashOutlet='$outlet' AND cashMonth='$month' ORDER BY sl DESC");
                                        		$i=1;
                                            ...
                                            <tr>
                                                <td></td>
                                                <td><?php echo $i++; ?></td>
                                                <td><?php echo $Total['cashDate']?></td>
                                                <td><?php echo $Total['endingBalance']?> <span>TK</span></td>
                                                <td><?php echo $Total['endingReserve']?> <span>TK</span></td>
                                                <td><?php echo $Total['totalCash']?> <span>TK</span></td>
                                                <td><?php echo $Total['expectedCash']?> <span>TK</span></td>
                                                <td class="bg-danger"><?php echo $Total['cashGap']?> <span>TK</span></td>
                                                <?php if($Total['cashStatus']=="Pending"){
                                                	?>
                                                <td>
			                                        <a onclick="javascript: return confirm('Please Confirm All Ok?');" href="inc/cashConfirm.php?sl=<?php echo $Total['sl']?>"><button data-toggle="tooltip" title="All okey" class="pd-setting-ed"><i class="fa fa-check" aria-hidden="true"></i></button></a>
			                                    </td>
                                           <?php  } ?>
