...
<html>
<head>
<title>Diana | <?php echo $_SESSION['outName'];?> | Product Mismatch</title>
            ...
            <!--header start here-->
				<div class="header-main">
					<div class="header-left" style="background-color: #00bfff; ">
							<div class="logo-nam">
									 <h1 class="text-center" style="padding: 5px; color: #fff"><b>Update Mismatch</b></h1> 
		...
		<p class="alert bg-success text-right">Date: <?php echo $date?></p>
							...
							<div class="panel panel-success" style="margin-top: 10px">
                                	<?php
                                		...
                                		if(isset($_POST['mismatch'])){
                                			$updatePrDistPrName = $_POST['updatePrDistPrName'];
                                			$updatePrDistQuantity = $_POST['updatePrDistQuantity'];
                                			$updatePrDistUnit = $_POST['updatePrDistUnit'];
                                			
                                			
                                			
                                			if(empty($updatePrDistPrName) || empty($updatePrDistQuantity)){
											$message = "<p class='alert alert-danger'>Field Must Not Be Empty</p>";

											}else{
												$pr = $conn->query("SELECT * FROM product WHERE prName = '$updatePrDistPrName' ");

												$prod = $pr->fetch_assoc();
												$prNameCheak = $prod['prName'];
												$prUnitCheak = $prod['prUnit'];

												$mi = $conn->query("SELECT * FROM productdistribution WHERE sl='$sl'");
                                    			$mis = $mi->fetch_assoc();
                                    			$Quantity = $mis['prDistQuantity'];
                                    			$Price = $mis['distTotalPrice'];
												
													
												
												
												$mismatch = "Mismatch";
												if($prNameCheak==$updatePrDistPrName AND $prUnitCheak== $updatePrDistUnit){
													 $distPerUnitPrice = $prod['prFixedPrice']/$prod['prQuantity'];
													 $distTotalPrice = $distPerUnitPrice*$updatePrDistQuantity;

													 if($Quantity>$updatePrDistQuantity){
													 	$mismatchQuantity = $Quantity-$updatePrDistQuantity ;
													 }if($Quantity<$updatePrDistQuantity){
													 	$mismatchQuantity =$updatePrDistQuantity- $Quantity;
													 }if($Price>$distTotalPrice){
													 	$mismatchPrice = $Price-$distTotalPrice ;

													 }if($Price<$distTotalPrice){
													 	$mismatchPrice = $distTotalPrice-$Price;
													 }
													 	
														

													$conn->query("UPDATE productdistribution SET prDistPrName='$updatePrDistPrName' , prDistQuantity= '$updatePrDistQuantity', prDistUnit='$updatePrDistUnit', distPerUnitPrice='$distPerUnitPrice', distTotalPrice = '$distTotalPrice', mismatchStatus='$mismatch', mismatchQuantity='$mismatchQuantity', mismatchPrice='$mismatchPrice',misMonth='$month',misYear='$year' WHERE sl=$sl");
													$conn->query("UPDATE productdistribution SET misYear='$year' WHERE misIndicate='indicate'");
											
												$message = "<p class='alert alert-success'>Successfully Updated Product! <a href='../proReceive.php?sl=$key'>Go to back Page</a></p>";
												}else{
													$message = "<p class='alert alert-danger'>Danger! Product & Unit Mismatch! <a href='../proReceive.php?sl=$key'>Go to back Page</a></p>";
												}
								...
								<form action="" method="POST">
									<?php
                                   	 $miss = $conn->query("SELECT * FROM productdistribution WHERE sl='$sl'");
                                    	$mism = $miss->fetch_assoc();
                                    	$Quantit = $mism['prDistQuantity'];
                                    	$Pric = $mism['distTotalPrice'];
                                    ?>
                                        <div class="row">
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                <div class="div">
                                                	<h3 class="alert bg-primary">Product Mismatch</h3>
                                                </div>
		                                            ...
		                                            <div class="chosen-select-single mg-b-20" style="margin-top: 8px;">
		                                                <label><i class="fa fa-list" aria-hidden="true"></i> <b>Product Name</b></label>
															...
															<input name="updatePrDistPrName"  value="<?php echo $mism['prDistPrName']?>" type="text" class="form-control" readonly>

		                                            </div>
                                                    <div class="form-group" style="margin-top: 8px;">
                                                    	<label for=""><i class="fa fa-calculator" aria-hidden="true"></i> Quantity</label>
                                                        
                                                        <input name="updatePrDistQuantity" value="<?php echo $Quantit?>" type="text" class="form-control">
                                                    </div>
                                                    <div class="chosen-select-single mg-b-20" style="margin-top: 8px;">
		                                                <label><i class="fa fa-balance-scale" aria-hidden="true"></i> Unit</label>
		                                                <select name="updatePrDistUnit" class="chosen-select" tabindex="-1">
																<option value="<?php echo $mism['prDistUnit'];?>" class="text-capitalize"><?php echo $mism['prDistUnit'];?></option>
																<?php
		                                            ...
		                                            <div class="form-group" style="margin-top: 8px;">
                                                    	<label for=""><i class="fa fa-money" aria-hidden="true"></i> Total Amount</label>
                                                        
                                                        <input name=""  value="<?php echo $Pric?>" type="text" class="form-control" readonly>
                                                    </div>
                                        ...
                                        <div class="row" style="margin-top: 8px">
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                <div class="text-center mg-b-pro-edt custom-pro-edt-ds">
                                                    <input name="mismatch" type="submit" value="Update Mismatch" class="btn btn-success">
